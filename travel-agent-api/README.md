...
Tieni il server API avviato in un terminale separato mentre usi il frontend.
...
**Il server non parte** → verifica di aver attivato l'ambiente Poetry con `eval $(poetry env activate)` prima di lanciare uvicorn.
...
**Python non compatibile** → assicurati di usare Python 3.9 o superiore (ma inferiore a 4.0). Verifica con `python --version`.